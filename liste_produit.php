...
<div class="row">
    <div class="col-md-3">
        <ul class="list-group bg-white text-black rounded p-2">
            ...
            <?php foreach ($categories as $idCategorie => $nomCategorie) { ?>
                <li class="list-group-item <?php if(isset($_GET["categorie"]) && $_GET["categorie"]==$idCategorie) echo 'active'; ?>">
    ...
    <div class="col-md-9">
        <div id="produits-container" class="row">
            ...
            produits.id_categorie, produits.disponibilite, tailles.id_taille, tailles.nom_taille, produits_tailles.prix_taille, produits_tailles.quantite
            FROM produits
            ...
            // Tableau pour stocker les produits et leurs tailles, prix, quantités
            $produits = array();
            ...
            if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) {
                    ...
                    $disponibiliteProduit = $row["disponibilite"];
                ...
                <?php foreach ($produits as $produit) { ?>
                    <div class="col-md-4 mb-3">
                        <div class="card h-100">
                            ...
                            <div class="card-body">
                                <h5 class="card-title" name="nom_produit"><b><?= $produit["nom_produit"] ?></b></h5>
                                ...
                                <ul class="list-group list-group-flush">
                                    <li class="list-group-item">
                                        <div class="form-group">
                                            <label for="taille"><b>Choisir la taille :</b></label>
                                    ...
                                    <li class="list-group-item">
                                        <?php if (isset($_SESSION["user_id"])) { ?>
                                            <?php if ($produit["disponibilite"] == "1") { ?>
                                                <button type="submit" name="ajout_panier" class="btn text-white  bi bi-plus-square" style="background-color: #21BA45; color: white;"> Ajouter au panier</button>
                                            <?php } else { ?>
                                                <button type="button" class="btn text-white bg-danger bi bi-exclamation-circle-fill" disabled> non-disponible</button>
                                            <?php } ?>
                                        <?php } else { ?>
                                            <span><b>Veuillez vous connecter pour ajouter au panier.</b></span>
                                        <?php } ?>
                                    </li>
                                </ul>
                            </form>
